...
1. Run `pnpm init nuxt@latest dashboard` (or manual setup if CLI not available)
2. Configure `nuxt.config.ts`:
   - Set Nitro preset to `node-server` (persistent runtime)
   - Configure source directory structure (`server/`, `app/`, `shared/`)
   - Add TypeScript strict mode
3. Install dependencies: `better-sqlite3`, `ws`, type definitions
4. Create directory structure:
   ...
   └── .env.example
   ```
5. Create `.env.example` with all required environment variables from requirements
6. Create minimal `app.vue` with a placeholder layout
7. Verify: `cd dashboard && pnpm install && pnpm run dev` starts without errors

### Skills & MCP Servers

| Resource | Purpose | When to Invoke |
|---|---|---|
| `nuxt` | Nuxt 4 + Nitro configuration | Server setup, config |
| `filesystem` (MCP) | File creation | Writing project files |
