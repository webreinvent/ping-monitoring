...
### `mapQualityState(qualityState: string): "good" | "degraded" | "poor" | "unknown"`

Maps the monitor row's `quality_state` to the API contract quality state.

```typescript
// Internal — not exported
function mapQualityState(qualityState: string): "good" | "degraded" | "poor" | "unknown"
```
...
- **Monitor with no samples:** The `LEFT JOIN` ensures the monitor still appears with `null` latest state fields. `qualityState` is `"unknown"` (from `warmingUp` mapping).
...
- **Unknown quality_state values:** Any value not in the known set (`"good"`, `"degraded"`, `"poor"`) maps to `"unknown"`. This future-proofs the function against quality state changes.
...
- [Shared Types](../shared/types.md) — `MonitorListItem` interface
